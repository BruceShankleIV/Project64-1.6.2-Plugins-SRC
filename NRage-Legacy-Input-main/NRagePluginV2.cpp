...
#ifdef _DEBUG
    sprintf(PluginInfo->Name,"Shankle's N-Rage's DirectInput8 (debug): %s",VER_FILE_VERSION_STR);
#else
    sprintf(PluginInfo->Name,"Shankle's N-Rage's DirectInput8 %s",VER_FILE_VERSION_STR);
#endif
...
EXPORT void CALL DllAbout ( HWND hParent )
{
    ...
    TCHAR szText[DEFAULT_BUFFER * 4] = _T(VER_FILE_DESCRIPTION_STR) _T("\n\n") \
        _T("Developed for Project64 1.6.2 by Edwin Bruce Shankle IV.\n\n") \
        _T("Version ") VER_FILE_VERSION_STR _T(" (") _T(__DATE__) _T(")\n") \
        _T("Made by N-Rage and many others.") \
        _T("\n") \
        _T(" - - - - -\n") \
...
if (iControl != -1 && g_pcControllers[iControl].fPlugged )
    {
